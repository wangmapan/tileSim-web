...
import { emitLightweightTelemetry, telemetryHash } from "../telemetry";

const props = defineProps<{
  adapter: LightweightAgentAdapter | null;
...
const statusText = computed(() => {
  if (props.loading) return "正在读取当前 capability snapshot…";
...
async function submit() {
  if (!props.adapter || !prompt.value.trim() || busy.value || props.loading) return;
  busy.value = true;
  emitLightweightTelemetry("lightweight_submit_started", {
    task_id: "prepare",
    context_revision_hash: telemetryHash(props.contextRevision || "lightweight-workbench"),
  });
  try {
    ...
    emitLightweightTelemetry("lightweight_result_state", {
      ...
      provenance_class: "agent_projection",
      latency_bucket: "local",
    });
  } finally {
    busy.value = false;
    await nextTick();
  ...
  <section class="lightweight-conversation" aria-labelledby="lightweight-conversation-title">
    ...
    <div class="lightweight-conversation__actions">
      ...
      ><button
        ...
        type="button"
        :disabled="!adapter || !prompt.trim() || busy || loading"
        @click="submit"
      >
    ...
    <div v-if="!adapter && !loading" class="lightweight-inline-error" role="alert">
