...
export function preserveWorkbenchQuery(
  query: Record<string, unknown>,
  fallbackRunId?: unknown,
): Record<string, string> {
  const carried: Record<string, string> = {};
  const run = firstQueryString(query.run);
  const fallback = firstQueryString(fallbackRunId);
  if (run && /^run-[\w-]+$/.test(run)) carried.run = run;
  else if (fallback && /^run-[\w-]+$/.test(fallback)) carried.run = fallback;
...
export const LIGHTWEIGHT_LIMITATIONS = [
  ...
  "Agent 不会自行创建正式运行；工作台提交不会调用 Provider，也不保存完整对话。",
] as const;
