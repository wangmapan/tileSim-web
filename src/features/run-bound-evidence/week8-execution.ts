...
export function buildWeek8ExecutionSummary(
  ...
  if (!report) {
    ...
    return empty(
      ...
      degraded === "not_applicable"
        ...
        : "当前 run 未提供 Week 8 execution artifact。",
    );
  }
  if (!runId || report.run_id !== runId)
    return empty("invalid_reference", "Week 8 artifact 的 run_id 与当前运行不一致。");
  ...
  return {
    ...
    reference: reference(manifest, "week8-run-evidence", "", "run", runId, "Week 8 execution artifact"),
