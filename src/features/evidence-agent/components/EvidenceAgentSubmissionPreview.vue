...
const evidenceSummary = computed(() =>
  isEnglish.value
    ...
    : `${props.citationLocationCount} 个精确引用位置，来自 ${props.citationArtifactCount} 份工件`,
);
const waitSummary = computed(() => {
  if (!Number.isFinite(props.timeoutMs) || props.timeoutMs <= 0) {
    return isEnglish.value
      ? "Service time limit unavailable; submission depends on service readiness."
      : "服务时限未提供；提交以服务就绪状态为准。";
  }
...
<template>
  ...
  >
    <header>
      <strong>{{ isEnglish ? "Evidence and execution bounds" : "证据与执行边界" }}</strong>
    </header>
    <dl>
      <div>
        <dt>{{ t("当前请求") }}</dt>
        <dd>
          ...
          <span v-else>{{ isEnglish ? "No request selected" : "未选择请求" }}</span>
        </dd>
      </div>
      <div>
