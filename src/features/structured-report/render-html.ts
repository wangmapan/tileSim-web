...
import {
  ...
  availabilityOfValue,
  availabilityTone,
} from "../report-coverage/model-api";
...
/**
 * The record status column, printed by the same rule as the on-screen record table
 * (`LayerRecordTable.vue:67-72`): a status the backend reported keeps its translated label and its own
 * tone, while a status it did not report — absent, empty, or the `unknown` placeholder — resolves to the
 * shared `missing` state instead of a bare dash. The screen side pins this with
 * `tests/components/layer-record-table.test.ts:67-79`.
 */
function renderStatusCell(record: ExecutionRecord) {
  const reported = record.status && record.status !== "unknown" ? record.status : null;
  if (reported) {
    return `<td class="availability availability--${statusTone(reported)}">${escapeHtml(statusLabel(reported))}</td>`;
  }
  const availability = availabilityOfValue(null);
  return `<td class="availability availability--${availabilityTone(availability)}" title="${escapeHtml(
    availabilityDescription(availability),
  )}">${escapeHtml(availabilityLabel(availability))}</td>`;
}
...
function renderRecords(records: ExecutionRecord[], title = t("结构化记录")) {
  ...
  return `<details><summary>${escapeHtml(title)} <span>${escapeHtml(t("{count} 条", { count: records.length }))}</span></summary>
    ...
    <tbody>${records
      .map(
        (record) =>
          `<tr><th><code>${escapeHtml(record.id)}</code><b>${escapeHtml(record.title)}</b>${record.subtitle ? `<small>${escapeHtml(record.subtitle)}</small>` : ""}</th>${renderStatusCell(record)}${columns
            .map((column) => {
              const fact = record.facts.find((item) => item.label === column);
              // Same resolver, label and tone as the on-screen record table (LayerRecordTable.vue):
              // an absent fact is `missing`, never a unified "不适用" placeholder, and the printed
              // cell keeps an inline description of that state.
              const availability = availabilityOfValue(fact?.value);
              const content =
                availability === "available" && fact
                  ? escapeHtml(display(fact.value, fact.unit))
                  : escapeHtml(availabilityLabel(availability));
              return `<td class="availability availability--${availabilityTone(availability)}" title="${escapeHtml(
                availabilityDescription(availability),
              )}">${content}</td>`;
            })
