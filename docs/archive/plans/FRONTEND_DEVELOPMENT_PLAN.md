...
稳定版候选的后续维护、复核和发布交接统一使用 `docs/plans/NEXT_STABLE_MAINTENANCE_PROMPT.md`。
...
详见 `docs/contracts/F8_EXPERIMENT_ORCHESTRATION_AUDIT.md`、`docs/contracts/F9_EVIDENCE_AGENT_CONTRACT_AUDIT.md`、
`docs/contracts/F9_EVIDENCE_AGENT_EVALUATION_SPEC.md` 和 `docs/archive/development/F7_FORMAL_CONTRACT_CLOSURE_2026-08-31.md`。
...
图型、来源、derivation、降级和包体记录见 `docs/architecture/EXECUTION_VISUALIZATION_DESIGN.md`。

发布机制闭合后的产品深化分为三个可并行工作流：Evidence Agent、可视化、页面帮助与逐步指引。三路任务的文件
所有权、共享冲突规则和集成顺序见 `docs/plans/PARALLEL_FRONTEND_WORKSTREAMS.md`；各自的历史阶段计划见
...
- **状态规则**：F10 release mechanics 已 validated。F9 的真实模型 acceptance 仍是独立发布 blocker，不能因 F10
  通过而升级。完整清单见 `docs/deployment/F10_RELEASE_HARDENING.md`。
...
- 结果字段与证据：`docs/features/execution/WEEK6_RESULTS_UI.md`
- Week 7 固定证据：`docs/features/execution/WEEK7_EVIDENCE_UI.md`
- F9 Agent 契约审计：`docs/contracts/F9_EVIDENCE_AGENT_CONTRACT_AUDIT.md`
- F9 Agent 评测规格：`docs/contracts/F9_EVIDENCE_AGENT_EVALUATION_SPEC.md`
