...
- Week 7 证据工作台：backend-global S8 offline fixture、S9 field evidence map 和确定性五步编排；详细边界见 `docs/WEEK7_EVIDENCE_UI.md`。
...
分层结果的图型、字段和降级规则见 `docs/EXECUTION_VISUALIZATION_DESIGN.md`。S0/S2 使用状态矩阵，不把分类状态数量化；S4 至少 3 个完整 latency–occupancy 样本才使用散点图；无数据时显示原因和字段表，不初始化空图。
