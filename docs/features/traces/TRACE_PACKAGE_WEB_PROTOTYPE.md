...
1. Bridge 从 `TILESIM_TRACE_PACKAGE_ROOT/<package-directory>/trace_package.json` 发现候选。正式工作台默认把
   `TILESIM_TRACE_PACKAGE_ROOT` 设置为被 Git 忽略的 `runtime/trace-packages`。
...
本地首次体验可以生成一个明确标记为 `synthetic_trace / uncalibrated / exploratory` 的确定性示例：

```powershell
.\scripts\initialize-trace-package-catalog.ps1
```

该示例只用于验证 Package 发现、完整性检查和运行链路，不构成真实系统证据。
