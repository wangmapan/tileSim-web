...
| `GAP-VALIDATE-001`          | open             | 后端领域服务                        | 统一 deterministic Validation Report                                                                                                            | profiles/calculators/lowering                       | 报告绑定 exact draft/profile/catalog/policy/backend/schema revision；发布 typed issues、versioned receipts、compiled request digest/Pointer、预算、claim-scope ceiling 和 hard gates         | 用 LLM 文本判断可行                                       |
...
| `GAP-PROFILE-SUCCESSOR-001` | blocked_data     | 各领域 Profile owner + Bridge       | v1 只允许五类空 Profile family，不能表达 Phase 2 所需 typed facts、完整 provenance 和生命周期                                                   | 经授权真实数据、source/license、calibration policy  | 发布版本化 successor 与兼容矩阵；区分 observed/inferred/modelled，声明 source/license/valid regime/expiry，支持 missing/expired/revoked/drift fail closed，并至少有一个经授权真实 record     | 在 v1 或前端静态 JSON 中填入产品宣传值                    |
| `GAP-CALCULATOR-001`        | blocked_backend  | 后端确定性规划领域服务 + 各资源模块 | weight/memory、KV、TP/PP/EP、placement、collective/network、workload distribution、typed SLO 七类 calculator/validator 无统一 versioned receipt | Profile successor、单位/舍入/unknown policy         | 每类输出 algorithm identity/revision、stable inputs、units、rounding、assumptions、uncertainty、rule IDs、repair candidates、claim ceiling；golden/property/boundary/risk-combination 全通过 | 用 LLM 或前端公式完成算术、排序或准入                     |
| `GAP-RUN-INTAKE-001`        | blocked_contract | Bridge + 场景与探索编排模块         | create-run v1 没有 Phase 2 compiled Profile/parallel/placement/KV/workload/SLO 输入                                                             | Profile/calculator/Validation Report、后端 lowering | 发布 create-run successor 或正式 workload intake 与兼容矩阵；unknown identity/revision fail closed，并有 request Pointer 到执行与 observable evidence 的累计测试                             | 把 Phase 1 本地草案或 trace_package_id 冒充正式 intake    |
