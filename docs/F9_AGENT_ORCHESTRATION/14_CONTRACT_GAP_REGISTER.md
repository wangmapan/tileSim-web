...
| `GAP-VALIDATE-001`          | blocked_backend | 后端领域服务                        | Validation Report v1 契约已发布；后端领域服务与 runtime 生成尚未实现                                                                                                 | profiles/calculators/lowering                       | 报告绑定 exact draft/profile/catalog/policy/backend/schema revision；发布 typed issues、versioned receipts、compiled request digest/Pointer、预算、claim-scope ceiling 和 hard gates         | 用 LLM 文本判断可行                                       |
...
| `GAP-PROFILE-SUCCESSOR-001` | blocked_data    | 各领域 Profile owner + Bridge       | 五类 Profile v2、field-level provenance、生命周期与组合 fail-closed 契约已正式发布；真实 Profile record 仍为空                                                       | 经授权真实数据、source/license、calibration policy  | 发布版本化 successor 与兼容矩阵；区分 observed/inferred/modelled，声明 source/license/valid regime/expiry，支持 missing/expired/revoked/drift fail closed，并至少有一个经授权真实 record     | 在 v1 或前端静态 JSON 中填入产品宣传值                    |
| `GAP-CALCULATOR-001`        | blocked_backend | 后端确定性规划领域服务 + 各资源模块 | 通用 envelope 与七类 typed receipt identity 已正式发布；正式算法、service 和执行证据仍未实现                                                                         | Profile successor、单位/舍入/unknown policy         | 每类输出 algorithm identity/revision、stable inputs、units、rounding、assumptions、uncertainty、rule IDs、repair candidates、claim ceiling；golden/property/boundary/risk-combination 全通过 | 用 LLM 或前端公式完成算术、排序或准入                     |
| `GAP-RUN-INTAKE-001`        | implementing    | Bridge + 场景与探索编排模块         | Run Intake v2 契约已正式发布，后端 parse/lowering 已提交，Bridge 侧只读预览端点（WP-2C-01a/01b）已注册；仍缺与 lowering 的执行路由、retained-run recovery 与累计证据 | Profile/calculator/Validation Report、后端 lowering | 发布 create-run successor 或正式 workload intake 与兼容矩阵；unknown identity/revision fail closed，并有 request Pointer 到执行与 observable evidence 的累计测试                             | 把 Phase 1 本地草案或 trace_package_id 冒充正式 intake    |
...
- `GAP-VALIDATE-001` 为 `blocked_backend`：Validation Report v1 契约已发布，但后端没有实现 runtime 生成；
- `GAP-PROFILE-SUCCESSOR-001`、`GAP-CALCULATOR-001`、`GAP-RUN-INTAKE-001` 阻塞完整 Phase 2；v2/receipt/intake 契约
  已发布，剩余阻塞是真实 Profile 数据、calculator 实现，以及 Bridge 侧到后端 lowering 的执行路由与累计证据；
...
Phase 2 checkpoint（复核日期：2026-09-17）：

- Phase 2A 的 proposal Schema、ADR、兼容矩阵、fixtures 与独立 oracle 已由 Phase 2B **正式发布并提交**（`4d7f9fa`）。
  发布物位于 `bridge/contracts/agent_orchestration_phase2/`，`runtime_status = contract_only`。发布不等于实现、
  接入 runtime 或 `validated`；
- 后端已提交 `parse_run_intake_v2` / `lower_run_intake_v2` 与只读 CLI 入口，因此
  `GAP-RUN-INTAKE-001` 进入 `implementing`：契约与 lowering 都在，缺 Bridge 路由和 request Pointer 到执行/observable
  evidence 的累计测试；
- `GAP-RUN-INTAKE-001` 的 Bridge 侧只读预览于 2026-09-17 落地（WP-2C-01a/01b，工作树未提交）：
  `registry.py` + `services/run_intake.py` + `POST /api/agent/run-intake-preview`。该端点补齐了「Bridge 侧路由 +
  runtime validator」的一半——它复用正式 validator、按冻结兼容矩阵判定并 fail closed，但**不调用后端 lowering**，
  也**不创建 run**，因此不产生 request Pointer 到执行/observable evidence 的累计证据。状态保持 `implementing`；
- `GAP-PROFILE-SUCCESSOR-001` 降级为 `blocked_data`：不再缺契约，缺的是经授权真实 Profile record、license/source
  审核与 lifecycle 证据；
- `GAP-CALCULATOR-001` 降级为 `blocked_backend`：envelope 与七类 typed receipt identity 已发布，正式算法、service
  与 golden/property/boundary/risk-combination 证据仍未实现；
- `GAP-VALIDATE-001` 降级为 `blocked_backend`：报告契约已发布，后端领域服务未实现；
- `GAP-PROFILE-SUCCESSOR-001` 最小剩余工作包为 runtime registry、canonical digest 与 missing/expired/revoked/drift
  negative corpus，以及至少一个经授权真实 record；
- `GAP-CALCULATOR-001` 最小剩余工作包为纯领域 calculator ports、规则库和 golden/property/boundary/risk-combination corpus；
- `GAP-RUN-INTAKE-001` 最小剩余工作包：Bridge 侧显式 nested routing 与 runtime validator 接线已在 WP-2C-01a/01b
  以**只读预览**形态交付，因此剩余为 —— 经 `WP-2C-02` 把后端 lowering 只读接进 Bridge 并原样透出 typed issue
  （遵守 Week 7 单槽约束）、retained-run recovery，以及从 intake Pointer 到后端执行与 observable evidence 的累计测试。
  详见 [Phase 2C Web 集成计划](23_PHASE2C_WEB_INTEGRATION_PLAN.md)。
...
2. `GAP-RUN-INTAKE-001`（Bridge 侧 Run Intake v2 到 lowering 的执行路由与累计证据）——契约、后端 lowering 与
   Bridge 只读预览都已就绪，是当前唯一无前置阻塞的 P0 实施项；
3. `GAP-CALCULATOR-001`、`GAP-VALIDATE-001` —— 契约已发布，等后端领域服务实现；
4. `GAP-PROFILE-SUCCESSOR-001` 与 `GAP-MODEL/DEVICE/ENGINE/PARALLEL/KV/WORKLOAD/NETWORK-*`
   —— 等经授权真实数据与执行闭包；
5. `GAP-DRAFT-001`，随后才是正式 Phase 2 Web receipt 展示；
6. `GAP-CONV/CLARIFY/APPROVAL-*`；
7. `GAP-WORKFLOW/CANCEL/OBS-*`；
8. `GAP-RAG/COMPARE/TOOLS/EVAL-*`；
9. MCP、GraphRAG、reviewer、A2A 等 deferred 项。
...
Phase 2 的**契约侧阻塞已解除**：2A 候选已由 2B 正式发布，后端已完成 intake v2 的 parse 与 lowering。Phase 2 的整体
状态仍不能标为 ready，因为数据侧（真实 Profile、calibration、held-out）与执行侧（calculator 实现、物理 KV、累计
lowering）仍为 `blocked_*`。

最近可安全推进的是把已发布的 Run Intake v2 契约接到现有 Bridge 与后端 lowering 之间：严格 nested routing、runtime
validator、typed issue 透出与 retained-run recovery，且**第一批不创建 run**。在其退出条件满足前，不新增
`agent_exposed` 字段，不启动 Web receipt UI。模型/设备/卡数、TP/PP/EP、placement、物理 KV、集合通信、网络与 SLO
只能展示已审计的 unsupported/unknown 状态，不能宣称可用。
